			...
			// Can't do this because several apps including ASP.NET trace
				...
				(function(window) {
					...
					setDocument = Sizzle.setDocument = function(node) {
						...
						contains = hasCompare || rnative.test(docElem.contains) ? function(
								a, b) {
						...
						pseudos : {
							...
							"not" : markFunction(function(selector) {
								...
								return matcher[expando] ? markFunction(function(
									...
									var elem, unmatched = matcher(seed, null,
											xml, []), i = seed.length;
				...
				Data.prototype = {
					...
					access : function(owner, key, value) {
						...
						if (key === undefined
							...
							return stored !== undefined ? stored : this.get(
									owner, jQuery.camelCase(key));
						}
						...
						.extend( {
							...
							dequeue : function(elem, type) {
								...
								var queue = jQuery.queue(elem, type), startLength = queue.length, fn = queue
										.shift(), hooks = jQuery._queueHooks(
										elem, type), next = function() {
				...
				jQuery.event = {
					...
					trigger : function(event, data, elem, onlyHandlers) {
						...
						var i, cur, tmp, bubbleType, ontype, handle, special, eventPath = [ elem
								...
								.call(event, "type") ? event.type : event, namespaces = hasOwn
					...
					dispatch : function(event) {
						...
						var i, j, ret, matched, handleObj, handlerQueue = [], args = slice
								.call(arguments), handlers = (data_priv.get(
					...
					fix : function(event) {
						...
						if (!fixHook) {
							this.fixHooks[type] = fixHook = rmouseEvent
									.test(type) ? this.mouseHooks : rkeyEvent
				...
				function getAll(context, tag) {
					...
					return tag === undefined || tag
							&& jQuery.nodeName(context, tag) ? jQuery.merge(
							[ context ], ret) : ret;
				}
				...
				jQuery.cssHooks.marginRight = addGetHookIf(
						support.reliableMarginRight, function(elem, computed) {
							if (computed) {
								// WebKit Bug 13343 - getComputedStyle returns wrong value for margin-right
				...
				function defaultPrefilter(elem, props, opts) {
					...
					var prop, value, toggle, tween, hooks, oldfire, display, anim = this, orig = {}, style = elem.style, hidden = elem.nodeType
							&& isHidden(elem), dataShow = data_priv.get(elem,
							"fxshow");
				...
				jQuery.fn.extend( {
					...
					undelegate : function(selector, types, fn) {
						...
						return arguments.length === 1 ? this
								.off(selector, "**") : this.off(types, selector
								|| "**", fn);
					}
				});
				...
				jQuery.each( [ "ajaxStart", "ajaxStop", "ajaxComplete",
						"ajaxError", "ajaxSuccess", "ajaxSend" ], function(i,
						type) {
					...
					if (self.length > 0) {
						jQuery.ajax( {
							...
							// Save response for use in complete callback
								...
								jQuery("<div>").append(
										jQuery.parseHTML(responseText)).find(
										selector)
										:
							...
							position : function() {
								...
								if (jQuery.css(elem, "position") === "fixed") {
									...
									parentOffset.top += jQuery.css(
											offsetParent[0], "borderTopWidth",
											true);
									parentOffset.left += jQuery.css(
											offsetParent[0], "borderLeftWidth",
											true);
								}
...
var moveDownFresh;
function moveUpOrFresh() {
	var totalHeight = document.body.scrollHeight;
	var topHeight = document.body.scrollTop;
	//	var pageHeight = document.documentElement.clientHeight;//预留
	var pageHeight = document.body.clientHeight;
	if (moveDownFresh && Math.abs(totalHeight - (topHeight + pageHeight)) < 50) {
		moveDownFresh();
	} else {
		moveUp();
	}
...
jQuery.extend({
      ...
      setCookie:function (cookieKey, cookieValue) {
    	  ...
    	  document.cookie= cookieKey + "=" + escape(cookieValue) + "; expires=" + date.toGMTString() + ";path=/"; 
      },
       // 获取cookie
...
var G = Object;
 G.alert = function(AC){
		Load(AC);
	 var t1 = window.setTimeout("dispalyLoad();",1000);
	 $(document).on("click",".datagrid-mask",function(){
		dispalyLoad();
	});
}
function Load(tt) {
    $("<div class=\"datagrid-mask\"></div>").css({ display: "block", width : "100%", height: $(window).height() , "background-color":"rgba(0,0,0,0.2)" , "position":"absolute",left: "0px", top: "0px","z-index":"998"}).appendTo("body");
    $("<div class=\"datagrid-mask-msg\"></div>").html("<div style=' position:absolute; top:40%; width:100%; text-align:center;'><div style='position:relative; width:80%; top:-50%; right:0; text-align:center;'>"+tt+"</div></div>").appendTo("body").css({display: "none", width : "80%",height:"100px", left: ($(document.body).outerWidth(true) - $(window).width()*0.8) / 2, top: ($(window).height() - 100) / 2  , "background-color":"#fff", "position":"absolute","z-index":"999"});
    $(".datagrid-mask-msg").fadeIn("slow");
}
	function dispalyLoad() {
    $(".datagrid-mask-msg").fadeOut("slow",function(){$(".datagrid-mask-msg").remove()});
    $(".datagrid-mask").fadeOut("slow",function(){$(".datagrid-mask").remove()});
}
